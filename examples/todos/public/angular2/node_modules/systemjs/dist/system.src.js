      ...
      for (var k = 0; k < load.dependencies.length; k++) {
        ...
        function asyncStartLoadPartwayThrough(stepState) {
          return function(resolve, reject) {
            ...
            if (loader.modules[name])
              throw new TypeError(
                '"' + name + '" already exists in the module table'
              );
              ...
              if (loader.loads[i].name == name) {
                ...
                if (step == 'translate' && !existingLoad.source) {
                  ...
                  proceedToTranslate(
                    loader,
                    existingLoad,
                    Promise.resolve(stepState.moduleSource)
                  );
                }
            ...
            if (step == 'locate') proceedToLocate(loader, load);
              ...
              proceedToFetch(
                loader,
                load,
                Promise.resolve(stepState.moduleAddress)
              );
            else {
        ...
        function updateLinkSetOnLoad(linkSet, load) {
          ...
          console.assert(
            load.status == 'loaded' || load.status == 'linked',
            'loaded or linked'
          );
        ...
        function linkSetFailed(linkSet, load, exc) {
          ...
          checkError: if (load) {
            if (linkSet.loads[0].name == load.name) {
              ...
              exc = addToError(
                exc,
                'Error loading ' + load.name + ' from ' + linkSet.loads[0].name
              );
            }
          } else {
        ...
        Loader.prototype = {
          ...
          newModule: function(obj) {
            ...
            else for (var key in obj) pNames.push(key);
...
*/
      ...
      var transpile = (function() {
        ...
        function typescriptTranspile(load, ts) {
          ...
          if (options.sourceMap) options.inlineSourceMap = true;
      ...
      function dedupe(deps) {
        ...
        for (var i = 0, l = deps.length; i < l; i++)
          if (indexOf.call(newDeps, deps[i]) == -1) newDeps.push(deps[i]);
        return newDeps;
      }
      ...
      (function() {
        ...
        function postExec() {
          if (--callCounter == 0) __global.System = curSystem;
          curLoad = undefined;
        }
        ...
        function getSource(load) {
          ...
          var wrap =
            ...
            load.metadata.bundle;

          return (
            (wrap ? '(function(System) {' : '') +
            load.source +
            (wrap ? '\n})(System);' : '') +
            ...
            (load.source.substr(lastLineIndex, 15) != '\n//# sourceURL='
              ? '\n//# sourceURL=' +
                load.address +
                (load.metadata.sourceMap ? '!transpiled' : '')
              : '') +
            // add sourceMappingURL if load.metadata.sourceMap is set
            ((load.metadata.sourceMap &&
              hasBtoa &&
              '\n//# sourceMappingURL=data:application/json;base64,' +
                btoa(unescape(encodeURIComponent(load.metadata.sourceMap)))) ||
              '')
          );
        }
        ...
        if (typeof document != 'undefined' && document.getElementsByTagName) {
          ...
          __exec = function(load) {
            if (!this.globalEvaluationScope) return evalExec.call(this, load);
...
*/
      ...
      function getBaseURLObj() {
        if (baseURLCache[this.baseURL]) return baseURLCache[this.baseURL];
        ...
        return function(name, parentName) {
          ...
          if (name[0] == '.' || name[0] == '/') {
            if (parentName)
              return new URL(
                name,
                parentName.replace(/#/g, '%05')
              ).href.replace(/%05/g, '#');
            else return new URL(name, baseURIObj).href;
          }
          return name;
        };
      });
      ...
      hook('import', function(systemImport) {
        return function(name, parentName, parentAddress) {
          if (parentName && parentName.name)
            warn.call(
              this,
              'System.import(name, { name: parentName }) is deprecated for System.import(name, parentName), while importing ' +
                name +
                ' from ' +
                parentName.name
            );
...
*/
      SystemJSLoader.prototype.config = function(cfg) {
        ...
        if (cfg.paths) {
          for (var p in cfg.paths) this.paths[p] = cfg.paths[p];
        }
          ...
          for (var p in cfg.map) {
            ...
            if (typeof v !== 'string') {
              ...
              for (var pkg in this.packages) {
                if (
                  normalized.substr(0, pkg.length) == pkg &&
              ...
              if (pkgMatch && this.packages[pkgMatch].main)
                normalized = normalized.substr(
                  0,
                  normalized.length - this.packages[pkgMatch].main.length - 1
                );

              var pkg = (this.packages[normalized] =
          ...
          if (objMaps)
            warn.call(
              ...
              'The map configuration for ' +
                objMaps +
                ' uses object submaps, which is deprecated in global map.\nUpdate this to use package contextual map with configs like System.config({ packages: { "' +
                p +
                '": { map: {...} } } }).'
            );
        }
        ...
        if (cfg.packageConfigPaths) {
          ...
          for (var i = 0; i < cfg.packageConfigPaths.length; i++) {
            ...
            var packageLength = Math.max(
              path.lastIndexOf('*') + 1,
              path.lastIndexOf('/')
            );
            var normalized = this.normalizeSync(
              path.substr(0, packageLength) + '/'
            );
            if (
              this.defaultJSExtensions &&
              path.substr(path.length - 3, 3) != '.js'
            )
              normalized = normalized.substr(0, normalized.length - 3);
            packageConfigPaths[i] =
              normalized.substr(0, normalized.length - 1) +
              path.substr(packageLength);
          }
          ...
          for (var p in cfg.packages) {
            ...
            if (cfg.packages[p].meta) {
              warn.call(
                ...
                'Package ' +
                  p +
                  ' is configured with meta, which is deprecated as it has been renamed to modules.'
              );
        ...
        for (var c in cfg) {
          ...
          if (typeof v != 'object' || v instanceof Array) {
            ...
            if (c == 'meta' || c == 'depCache') normalizeProp = true;

            for (var p in v) {
              if (c == 'meta' && p[0] == '*') this[c][p] = v[p];
              else if (normalizeProp) this[c][this.normalizeSync(p)] = v[p];
              else this[c][p] = v[p];
            }
          }
        }
      ...
      hook('normalize', function(normalize) {
        return function(name, parentName) {
          ...
          if (normalized.match(absURLRegEx)) {
            ...
            if (
              this.defaultJSExtensions &&
              normalized.substr(normalized.length - 3, 3) != '.js'
            )
              normalized += '.js';
            return normalized;
          }
      ...
      (function() {
        ...
        function getBasePath(pkg) {
          // sanitize basePath
          var basePath =
            pkg.basePath && pkg.basePath != '.' ? pkg.basePath : '';
          if (basePath) {
            if (basePath.substr(0, 2) == './') basePath = basePath.substr(2);
        ...
        function toPackagePath(
          ...
          var skipExtension = !!(
            isPlugin ||
            subPath.indexOf('#?') != -1 ||
            subPath.match(interpolationRegEx)
          );
          ...
          var normalized =
            ...
            basePath +
            subPath +
            (skipExtension ? '' : getDefaultExtension(pkg, subPath));
        ...
        function getDefaultExtension(pkg, subPath) {
          ...
          if (
            ...
            if (
              subPath.substr(subPath.length - defaultExtension.length) !=
              defaultExtension
            )
        ...
        function applyPackageConfig(normalized, pkgName, pkg, sync, isPlugin) {
          ...
          if (pkgName === normalized && pkg.main)
            normalized +=
              '/' +
              (pkg.main.substr(0, 2) == './' ? pkg.main.substr(2) : pkg.main);

          // allow for direct package name normalization with trailling "/" (no main)
          if (
            normalized.length == pkgName.length + 1 &&
            normalized[pkgName.length] == '/'
          )
            return normalized;
          ...
          if (pkg.map) {
            ...
            var map =
              applyMap(pkg.map, subPath) ||
              (!isPlugin &&
                applyMap(
                  pkg.map,
                  (subPath += getDefaultExtension(pkg, subPath.substr(2)))
                ));
          ...
          function doMap(mapped) {
            ...
            // global package map
              ...
              return (sync ? loader.normalizeSync : loader.normalize).call(
                loader,
                mapped
              );
          }
          ...
          if (sync || !mapped)
            return toPackagePath(
              ...
              pkg,
              basePath,
              normalized.substr(pkgName.length + 1),
              sync,
              isPlugin
            );
          ...
          if (loader.builder) return pkgName + '#:' + map.substr(2);
        ...
        function createPackageNormalize(normalize, sync) {
          return function(name, parentName, isPlugin) {
            ...
            if (parentName)
              var parentPackage =
                getPackage.call(this, parentName) ||
                (this.defaultJSExtensions &&
                  parentName.substr(parentName.length - 3, 3) == '.js' &&
                  getPackage.call(
                    this,
                    parentName.substr(0, parentName.length - 3)
                  ));
            ...
            if (parentPackage) {
              ...
              if (
                ...
                parentName.substr(
                  ...
                  parentBasePath.length
                ) == parentBasePath
              )
                parentName =
                  parentPackage +
                  parentName.substr(
                    parentPackage.length + parentBasePath.length
                  );
              ...
              if (name[0] !== '.') {
                ...
                if (parentMap) {
                  ...
                  if (map) {
                    ...
                    if (name[0] === '.') parentName = parentPackage + '/';
                  }
                }
              }
            }
            ...
            function packageResolution(normalized, pkgName, pkg) {
              ...
              if (pkg)
                return applyPackageConfig.call(
                  loader,
                  normalized,
                  pkgName,
                  pkg,
              ...
              else return normalized + (defaultJSExtension ? '.js' : '');
            }

            // do direct resolution if sync
            if (sync) return packageResolution(normalized);
            ...
            return (
              Promise.resolve(getBundleFor(loader, normalized))
                ...
                .then(function(bundle) {
                  if (bundle || pkgBundlePromises[pkgConfigMatch.pkgName]) {
                    ...
                    if (
                      bundle &&
                      indexOf.call(pkgBundleLoads.bundles, bundle) == -1
                    ) {
        ...
        function pkgConfigPathMatch(loader, normalized) {
          ...
          for (var i = 0; i < loader.packageConfigPaths.length; i++) {
            ...
            var match = normalized.match(
              packageConfigPathsRegExps[p] ||
                (packageConfigPathsRegExps[p] = new RegExp(
                  '^(' +
                    p.substr(0, pPkgLen).replace(/\*/g, '[^\\/]+') +
                    ')(/|$)'
                ))
            );
        ...
        function loadPackageConfigPaths(loader, pkgConfigMatch) {
          ...
          if (curPkgConfig && curPkgConfig.configured) return Promise.resolve();
                    ...
                    pkgConfigPromises.push(
                      loader['fetch']({
                        ...
                        .then(function(cfg) {
                          ...
                          // support main array
                          if (cfg.main instanceof Array) cfg.main = cfg.main[0];

                          // deeply-merge (to first level) config with any existing package config
                          if (curPkgConfig) extendMeta(cfg, curPkgConfig);

                          // support external depCache
                          if (cfg.depCache)
                            for (var d in cfg.depCache) {
                              if (d.substr(0, 2) == './') continue;

                              var dNormalized = loader.normalizeSync(d);
                              loader.depCache[dNormalized] = (
                                loader.depCache[dNormalized] || []
                              ).concat(cfg.depCache[d]);
                            }

                          curPkgConfig = loader.packages[
                            pkgConfigMatch.pkgName
                          ] = cfg;
                        })
                    );
        ...
        function getMetaMatches(pkgMeta, pkgName, subPath, matchFn) {
          ...
          for (var module in pkgMeta) {
            ...
            if (
              module.substr(0, wildcardIndex) ==
                subPath.substr(0, wildcardIndex) &&
              module.substr(wildcardIndex + 1) ==
                subPath.substr(
                  subPath.length - module.length + wildcardIndex + 1
                )
            ) {
              matchFn(
                module,
                pkgMeta[dotRel + module],
                module.split('/').length
              );
            }
          }
          ...
          return function(load) {
            ...
            return Promise.resolve(locate.call(this, load)).then(function(
              ...
              if (pkgName) {
                ...
                var basePath = getBasePath(pkg);
                var subPath = load.name.substr(
                  pkgName.length + basePath.length + 1
                );
      ...
      (function() {
        ...
        var ieEvents =
          ...
          (function() {
            ...
            return (
              s.attachEvent &&
              !(
                s.attachEvent.toString &&
                s.attachEvent.toString().indexOf('[native code') < 0
              ) &&
              !isOpera
            );
          })();
        ...
        function getInteractiveScriptLoad() {
          ...
          for (var i = 0; i < interactiveLoadingScripts.length; i++)
            if (
              interactiveLoadingScripts[i].script.readyState == 'interactive'
            ) {
        ...
        hook('pushRegister_', function(pushRegister) {
          return function(register) {
            ...
            else if (loadingCnt) registerQueue.push(register);
        ...
        function webWorkerImport(loader, load) {
          return new Promise(function(resolve, reject) {
            ...
            if (!load.metadata.entry)
              reject(
                new Error(
                  load.address + ' did not call System.register or AMD define'
                )
              );
            ...
            return new Promise(function(resolve, reject) {
              ...
              function complete(evt) {
                ...
                if (!load.metadata.entry && !load.metadata.bundle)
                  reject(
                    new Error(
                      load.name + ' did not call System.register or AMD define'
                    )
                  );
      ...
      (function() {
        ...
        SystemJSLoader.prototype.register = function(name, deps, declare) {
          ...
          entry.name =
            name && (this.normalizeSync || this.normalize).call(this, name);
        ...
        hook('reduceRegister_', function() {
          return function(load, register) {
            ...
            if (!entry.name || (load && entry.name == load.name)) {
              if (!curMeta)
                throw new TypeError(
                  'Unexpected anonymous System.register call.'
                );
              if (curMeta.entry) {
        ...
        function link(name, loader) {
          ...
          var curGroupDeclarative =
            !!startEntry.declarative == groups.length % 2;
        ...
        function linkDeclarativeModule(entry, loader) {
          ...
          var module = (entry.module = getOrCreateModuleRecord(
            entry.name,
            moduleRecords
          ));
          ...
          if (!module.setters || !module.execute) {
            throw new TypeError(
              'Invalid System.register form for ' + entry.name
            );
          }
        ...
        function getModule(name, loader) {
          ...
          if (!entry) {
            ...
            if (entry.declarative) ensureEvaluated(name, [], loader);
            else if (!entry.evaluated) linkDynamicModule(entry, loader);
        ...
        function linkDynamicModule(entry, loader) {
          ...
          if (exports && exports.__esModule) entry.esModule = exports;
        ...
        function ensureEvaluated(moduleName, seen, loader) {
          ...
          for (var i = 0, l = entry.normalizedDeps.length; i < l; i++) {
            ...
            if (indexOf.call(seen, depName) == -1) {
              ...
              else ensureEvaluated(depName, seen, loader);
            }
          }
        ...
        function detectRegisterFormat(source) {
          ...
          return (
            ...
            source.substr(leadingCommentAndMeta[0].length, 15) ==
              'System.register'
          );
        }
        ...
        hook('instantiate', function(instantiate) {
          return function(load) {
            ...
            // (dont run bundles in the builder)
              ...
              if (typeof __exec != 'undefined') __exec.call(loader, load);
          ...
          return function(load) {
            ...
            return translate.call(loader, load).then(function(source) {
              ...
              if (
                load.metadata.format == 'esm' ||
                load.metadata.format == 'es6' ||
                (!load.metadata.format && source.match(esmRegEx))
              ) {
                ...
                if (loader.transpiler === false)
                  throw new TypeError(
                    'Unable to dynamically transpile ES module as System.transpiler set to false.'
                  );
                ...
                if (loader.pluginLoader)
                  loader.pluginLoader.loadedTranspiler_ =
                    loader.loadedTranspiler_ || false;
              ...
              if (
                ...
                warn.call(
                  loader,
                  'Note that internal transpilation via System.transpiler has been deprecated for transpiler plugins.'
                );
              ...
              if (loader.loadedTranspilerRuntime_ === false) {
                if (
                  load.name == loader.normalizeSync('traceur-runtime') ||
              ...
              if (
                (load.metadata.format == 'register' || load.metadata.bundle) &&
                loader.loadedTranspilerRuntime_ !== true
              ) {
                if (
                  !__global.$traceurRuntime &&
                  load.source.match(traceurRuntimeRegEx)
                ) {
                ...
                if (
                  !__global.babelHelpers &&
                  load.source.match(babelHelpersRegEx)
                ) {
                  loader.loadedTranspilerRuntime_ =
                    loader.loadedTranspilerRuntime_ || false;
...
*/
      ...
      hook('reduceRegister_', function(reduceRegister) {
        return function(load, register) {
          ...
          var globalValue = readMemberExpression(
        ...
        return function(load) {
          ...
          if (load.metadata.format == 'global' && !load.metadata.registered) {
            ...
            entry.execute = function(require, exports, module) {
              ...
              var retrieveGlobal = loader
                .get('@@global-helpers')
                .prepareGlobal(module.id, exportName, globals);
      ...
      hookConstructor(function(constructor) {
        return function() {
          ...
          function forEachGlobal(callback) {
            if (Object.keys) Object.keys(__global).forEach(callback);
            else
              for (var g in __global) {
                if (!hasOwnProperty.call(__global, g)) continue;
                callback(g);
              }
          }
            ...
            loader.newModule({
              prepareGlobal: function(moduleName, exportName, globals) {
                ...
                return function() {
                  ...
                  // revert globals
                  if (oldGlobals) {
                    for (var g in oldGlobals) __global[g] = oldGlobals[g];
                  }
      ...
      (function() {
        ...
        function getCJSDeps(source) {
          ...
          function inLocation(locations, match, starts) {
            ...
            for (var i = 0; i < locations.length; i++)
              if (
                locations[i][0] < match.index &&
                locations[i][1] > match.index + (!starts ? match[0].length : 0)
              )
                return true;
            return false;
          }
        ...
        hook('normalize', function(normalize) {
          return function(name, parentName) {
            ...
            if (
              name.substr(0, 6) == '@node/' &&
              nodeCoreModules.indexOf(name.substr(6)) != -1
            ) {
              if (!this._nodeRequire)
                throw new TypeError('Can only load node core modules in Node.');
              this.set(
                name,
                this.newModule(getESModule(this._nodeRequire(name.substr(6))))
              );
            }
        ...
        hook('instantiate', function(instantiate) {
          return function(load) {
            ...
            if (!load.metadata.format) {
              ...
              if (
                cjsRequireRegEx.exec(load.source) ||
                cjsExportsRegEx.exec(load.source)
              )
            ...
            if (load.metadata.format == 'cjs') {
              ...
              entry.execute = function(require, exports, module) {
                ...
                if (address.substr(0, 8) == 'file:///') {
                  ...
                  windowOrigin &&
                  address.substr(0, windowOrigin.length) === windowOrigin
                ) {
    ...
    */
          ...
          function define(name, deps, factory) {
            ...
            if ((requireIndex = indexOf.call(deps, 'require')) != -1) {
              ...
              if (!name)
                deps = deps.concat(
                  getCJSDeps(factory.toString(), requireIndex)
                );
            }
            ...
            function execute(req, exports, module) {
              ...
              var output = factory.apply(
                exportsIndex == -1 ? __global : exports,
                depValues
              );
            ...
            entry.name =
              name &&
              (loader.normalizeSync || loader.normalize).call(loader, name);
            entry.deps = deps;
            entry.execute = execute;
          ...
          hook('reduceRegister_', function(reduceRegister) {
            return function(load, register) {
              ...
              if (!entry.name) {
                ...
                if (curMeta.entry)
                  throw new TypeError(
                    'Multiple defines for anonymous module ' + load.name
                  );
      ...
      (function() {
        ...
        hook('instantiate', function(instantiate) {
          return function(load) {
            ...
            if (
              load.metadata.format == 'amd' ||
              (!load.metadata.format && load.source.match(amdRegEx))
            ) {
              load.metadata.format = 'amd';
...
*/
        ...
        function normalizePlugin(normalize, name, parentName, isPlugin, sync) {
          ...
          if (pluginIndex != -1) {
            ...
            var defaultExtension =
              loader.defaultJSExtensions &&
              argumentName.substr(argumentName.length - 3, 3) != '.js';
      ...
      function parseCondition(condition) {
        ...
        if (conditionExportIndex != -1) {
          ...
          conditionModule =
            condition.substr(negation, conditionExportIndex - negation) ||
            '@system-env';
        } else {
      ...
      function serializeCondition(conditionObj) {
        return (
          (conditionObj.negate ? '~' : '') +
          conditionObj.module +
          (conditionObj.prop ? '|' + conditionObj.prop : '')
        );
      }
      ...
      function resolveCondition(conditionObj, parentName, bool) {
        return this['import'](conditionObj.module, parentName).then(function(
          ...
          else if (typeof m == 'object' && m + '' == 'Module') m = m['default'];
      ...
      function interpolateConditional(name, parentName) {
        ...
        var conditionObj = parseCondition(
        ...
        if (this.builder)
          return this['normalize'](conditionObj.module, parentName).then(
            function(conditionModule) {
              ...
              return name.replace(
                interpolationRegEx,
                '#{' + serializeCondition(conditionObj) + '}'
              );
            }
          );
      ...
      function booleanConditional(name, parentName) {
        ...
        if (booleanIndex == -1) return Promise.resolve(name);
...
*/
      ...
      (function() {
        ...
        hook('locate', function(locate) {
          return function(load) {
            ...
            if (deps)
              for (var i = 0; i < deps.length; i++) loader['import'](deps[i]);
      ...
      if (typeof exports === 'object') module.exports = Loader;
      ...
      if (typeof exports === 'object') module.exports = System;
